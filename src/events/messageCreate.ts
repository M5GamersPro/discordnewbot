...
export async function handleMessage(message: Message) {
  ...
  if (gained?.leveledUp && message.channel.isSendable()) {
    await message.channel.send(`🎉 ${message.author}, you reached **level ${gained.level}**!`).catch(() => undefined);
  }
  const config = await prisma.autoModConfig.findUnique({ where: { guildId: message.guild.id } }).catch(() => null);
